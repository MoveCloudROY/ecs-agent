...
async def test_hashline_basic_operations(tmp_path: Path) -> None:
    ...
    result = await edit_file(
        "sample.txt",
        json.dumps([{"op": "replace", "pos": f"2#{beta_hash}", "lines": ["BETA"]}]),
        str(workspace),
    )

    assert result == "Applied 1 edits to sample.txt"
...
async def test_hashline_append_and_prepend(tmp_path: Path) -> None:
    ...
    await edit_file(
        ...
        json.dumps(
            [
                {"op": "append", "pos": f"2#{banana_hash}", "lines": ["grape"]},
                {"op": "prepend", "pos": f"1#{apple_hash}", "lines": ["start"]},
            ]
        ),
        str(workspace),
    )
...
async def test_hashline_batch_two_replacements_single_call(tmp_path: Path) -> None:
    ...
    result = await edit_file(
        ...
        json.dumps(
            [
                {"op": "replace", "pos": f"1#{red_hash}", "lines": ["crimson"]},
                {"op": "replace", "pos": f"3#{blue_hash}", "lines": ["navy"]},
            ]
        ),
        str(workspace),
    )

    assert result == "Applied 2 edits to batch.txt"
...
async def test_hashline_duplicate_line_targeting_by_anchor(tmp_path: Path) -> None:
    ...
    await edit_file(
        "dupes.txt",
        json.dumps([{"op": "replace", "pos": f"3#{line3_hash}", "lines": ["CHANGED"]}]),
        str(workspace),
    )
            ...
            [
                {
                    ...
                    "pos": f"2#{todo_hash}",
                    "end": f"2#{todo_hash}",
                    "lines": ["step 1: init", "step 2: process", "step 3: cleanup"],
                }
            ]
        ),
        str(workspace),
