...
async def test_background_subagent_completion_strips_envelope_before_persisting_artifact(
    tmp_path: Path,
) -> None:
    ...
    wrapped_result = (
        ...
        f"<full_result>{full_result}</full_result>"
        "</subagent_background_result>"
    )
            ...
            subagents={
                "artifact-agent": SubagentConfig(model=FakeModel(
                        responses=[
                            CompletionResult(
                                message=Message(
                                    role="assistant", content=wrapped_result
                                )
                            )
                        ]
                    ),
name="artifact-agent",
    ...
    table = world.get_component(parent, SubagentSessionTableComponent)
    assert table is not None
    assert table.sessions[session_id].result_summary == summary
