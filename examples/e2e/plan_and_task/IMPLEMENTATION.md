...
## Testing
  ...
  - **Integration Tests**: `tests/integration/test_plan_and_task_flow.py` covers the command surface, state machine, artifact persistence, and credential-gated CLI checks without depending on `FakeModel`.
 ...
 - **Live Tests**: `tests/live/test_plan_and_task_flow_live.py` provides credential-gated acceptance tests using a real LLM model backend. Includes `test_live_derive_workflow_id_from_llm_returns_valid_slug` which verifies the LLM returns a valid `^[a-z][a-z0-9-]*$` slug (3–50 chars) for a real Chinese task description.
