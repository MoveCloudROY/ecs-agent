...
- `/plan:write`: Transition from `DRAFT_QA_REVIEW` to `WRITE_PLAN` phase to produce `workflow_plan.md`. **Optional** — the `DRAFT_QA_REVIEW` `ApprovalGate` performs this transition automatically when QA approves the draft. Manual invocation mirrors that gate: it only yields to `WRITE_PLAN` once the latest `DRAFT_QA_REVIEW` verdict is `approved`, and refuses (with an error, leaving the phase put) while QA is still on `revise`/`blocked` — it never bypasses the gate.
