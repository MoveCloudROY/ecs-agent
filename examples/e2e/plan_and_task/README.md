...
- **Verdict Extraction**: Subagent results are parsed by scanning for the first case-insensitive match of a verdict word — `approved`, `revise`, or `blocked` (regex `\b(approved|revise|blocked)\b`). If no verdict word is found, extraction logs `plan_task_verdict_extraction_failed` and defaults to `revise`.
