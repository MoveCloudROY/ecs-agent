...
5. **Execution**: Once finalized, the plan is decomposed into a task queue (`/task:start`). The main agent executes the active task with its file/bash tools, verifies every acceptance criterion, then calls the `complete_task` tool to record completion — which writes an evidence record and advances the live queue to the next task, or to `TASK_COMPLETED` when none remain.
...
- **GitHub CLI Skill (optional)** — The official [`gh` skill](https://github.com/cli/cli/tree/trunk/skills/gh) (`.claude/skills/gh/SKILL.md`) teaches the agent the `gh` CLI's non-obvious patterns: `--json`/`--jq` output, pagination caps, search-vs-list, sub-issues/discussions, and the `gh api` fallback. It is a **pure-instruction markdown skill** — it carries no tools of its own; the agent runs `gh` through the built-in `bash` tool. When the `gh` binary is present, `build_plan_task_world` installs it on the main agent; it is listed by name under `${_installed_skills}` in the PLAN and TASK prompts, and the model pulls the full guide on demand with `load_skill_details("gh")` before touching GitHub. Progressive disclosure keeps the ~9 KB guide out of every turn's context. Requires the `gh` binary to be installed and authenticated (`gh auth login` or `GH_TOKEN`/`GITHUB_TOKEN`); when it is absent the skill is omitted and the workflow runs unchanged.
...
- **Task Execution**: `TaskExec` handles plan loading and dependency-ordered queue construction (`/task:start`). During `TASK_RUNNING` the main agent executes each active task itself and calls the `complete_task` tool (`task_tools.py`) to record completion — wrapping `TaskExec.record_task_completion`, which writes the evidence record, appends a memory entry, and advances `current_task_id` to the next pending task or transitions to `TASK_COMPLETED`.
...
- `/plan:finalize`: Finalize and transition to task execution (requires all three approved reviews). It validates the approvals, confirms the `plan_writer`'s `workflow_plan.md` exists, and walks the phase to `TASK_READY` — it does **not** rewrite the plan (the task queue is built from the plan_writer's document).
- `/plan:write`: Produce `workflow_plan.md` by (re)triggering the `plan_writer`. **Optional** — the `DRAFT_QA_REVIEW` `ApprovalGate` advances to `WRITE_PLAN` and triggers the writer automatically when QA approves. Manual invocation mirrors that gate: from `DRAFT_QA_REVIEW` it yields to `WRITE_PLAN` only once the latest verdict is `approved` (refusing, phase put, while QA is on `revise`/`blocked`); and because approval auto-advances the phase, from `WRITE_PLAN` it is an idempotent success that simply re-emits the write-plan trigger so the writer re-runs.
...
- `GH_TOKEN` / `GITHUB_TOKEN`: Optional GitHub auth for the `gh` CLI skill. The skill is installed whenever the `gh` binary is on `PATH`; `gh` picks up auth from these env vars or from `gh auth login`. No token is needed to install the skill, but `gh` commands the agent runs will fail without valid auth.
...
- **Circuit Breaker**: a same-scope `/task:replan` counts as a failed attempt and increments the active task's `TaskRecord.retry_count`; once it reaches the retry limit (`_MAX_TASK_RETRIES`, 3) `TaskExec.check_circuit_breaker` trips and the workflow holds the task in `TASK_BLOCKED` instead of retrying forever. A `--scope-changed` replan resets the count (fresh plan). Recover a blocked task by replanning with `--scope-changed` (re-review) or `/task:abort`.
