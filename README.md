...
- **Phase Graphs** — Explicit command-driven phase machine: pure-data graphs (`build_graph`), async `advance()/force()/record_approval()` transitions with bounded audit history, first-class approval gates, per-phase tool allowlists and resume policy, loud checkpoint-restore validation.
...
- **Built-in Tools** — `BuiltinToolsSkill` provides `read_file`, `write_file`, `edit_file`, `bash`, `glob`, `grep`, `explore`, `webfetch`, `code_execution`, and `interactive_bash` with workspace binding, path traversal protection, clean file reads, ECS-scoped snapshot-protected editing, and persistent tmux session support. File snapshots are framework-managed through `ToolExecutionContext` and `ToolRuntimeStateComponent`, so hashes and snapshot IDs stay out of LLM-visible tool calls. Read-only tools (`read_file`, `grep`, `glob`, `explore`, `webfetch`) are marked `concurrency_safe` and execute in parallel when the model batches them.
...
- **Prompt Caching** — Every outbound path keeps prompts **append-only** so provider prefix caches (Anthropic `cache_control`, OpenAI Chat/Responses automatic caching) hit call-over-call: the system prompt splits into a byte-stable prefix and a volatile tail (compaction summary + phase prompt), rendered user prompts freeze into history when the turn advances, and context-pool/slash-skill blocks ride a trailing user message persisted on commit. The Claude adapter emits up to 4 `cache_control` breakpoints — tool definitions, the stable system prefix, the latest message, plus a ladder marker within the ~20-block lookback window so wide parallel tool batches don't strand the previous cache entry. Enabled by default; toggle with `Model(..., enable_prompt_caching=False)`. Verify with the offline prefix audit (`tests/test_prompt_cache_prefix_audit.py`) and live experiments (`tests/live/test_prompt_cache_experiment_live.py`); see [`docs/features/prompt-caching.md`](docs/features/prompt-caching.md).
- **Tool Ecosystem** — Auto-discovery via `@tool` decorator, concurrent execution of batched tool calls (opt-in per tool via `ToolSchema.concurrency_safe`; results land in original order, mutating tools stay serial barriers, bound configurable with `ToolExecutionConfigComponent(max_concurrency=...)`), manual approval flows, secure `bwrap` sandboxing, and composable skills.
...
| [`examples/e2e/plan_and_task/`](examples/e2e/plan_and_task/) | Interactive plan→review→execute workflow; recoverable phase graph (`ecs_agent.phases`), review-gated planning, artifact persistence, slash-command dispatch, and framework-native auto compaction across main/subagent runs. Ships a Textual TUI (`python -m examples.e2e.plan_and_task.tui`, `tui` extra) rendering the event bus live: streaming transcript, phase/task/review sidebar, and token-usage bar |
