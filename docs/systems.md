...
## 1c. Phase Graphs (no system)

Phase transitions are explicit async calls — `ecs_agent.phases.advance()`, `ecs_agent.phases.force()`, and `ecs_agent.phases.record_approval()` — rather than a polling system, so there is no registration priority and no system-ordering contract. Per-phase prompts resolve through the `PhasePromptPlaceholderProvider` during `SystemPromptRenderSystem` rendering (`${_phase_prompt}`). See [`features/phases.md`](./features/phases.md) for the full model.
...
- **Constructor**: `__init__(self, priority: int = 0, scratchbook_service: ScratchbookService | None = None, registry: ArtifactRegistry | None = None)`
- **Queries**: `PendingToolCallsComponent`, `ToolRegistryComponent`, `ConversationComponent`
- **Reads**: `ToolExecutionConfigComponent` (concurrency bound), `SandboxConfigComponent`, `ContextTrimConfig`, `PlanComponent`
...
- **Events Published**: `ToolExecutionStartedEvent`, `ToolExecutionCompletedEvent`, `ToolResultCachedEvent`.
- **Recommended Priority**: 5

### Behavior
The system splits each entity's pending batch into ordered execution groups. Consecutive calls whose `ToolSchema.concurrency_safe` is `True` form one group and run **concurrently** (bounded by `ToolExecutionConfigComponent.max_concurrency`, default 8); every other call — mutating tools, unknown tools — forms a singleton group that acts as a **barrier**: it starts only after all earlier calls finished and blocks later calls until it returns.

Results always land in the original `tool_calls` order: conversation appends, scratchbook persistence, and plan status updates run serially after each group completes, so message order (and prompt-cache prefixes) stay deterministic regardless of completion order.

### Concurrency contract
- `ToolSchema.concurrency_safe = True` declares that a tool has no side effects conflicting with other tools in the same batch (read-only file access, pure lookups). The `@tool` decorator accepts the same flag: `@tool(description=..., concurrency_safe=True)`.
- Built-in read-only tools (`read_file`, `grep`, `glob`, `explore`, `webfetch`) and `subagent_status` are marked safe; mutating tools (`write_file`, `edit_file`, `bash`, `interactive_bash`, `code_execution`) default to serial.
- `ToolExecutionConfigComponent(max_concurrency=1)` restores fully serial execution for an entity; without the component the bound defaults to 8.
- `ToolExecutionStartedEvent` / `ToolExecutionCompletedEvent` may interleave across a concurrent group; consumers pair them by `tool_call_id`.

### Error Handling
This system does not throw exceptions. If it encounters an unknown tool or a handler fails, it records the error as a string within the tool result message so the LLM can respond to the failure. Task cancellation (`asyncio.CancelledError`) propagates and cancels the remaining calls of the in-flight group.
...
# Optional: bound (or disable) concurrent execution for one entity
...
- **Constructor**: `__init__(self, context_providers=None)` — optional `Sequence[CompactionContextProvider]`; defaults to `DEFAULT_COMPACTION_CONTEXT_PROVIDERS` (subagent session state, todo list).
...
### Context providers
State that must survive compaction (e.g. pending subagent sessions) is contributed to the summarization request through the `CompactionContextProvider` protocol (`ecs_agent.prompts.compaction_context`): each provider returns a text block or `None`, and non-empty blocks are appended to the summary input in provider order. CompactionSystem has no knowledge of any contributor's domain — the built-in `SubagentCompactionContextProvider` owns the subagent digest, classifying sessions with the canonical status sets from `ecs_agent.types` (`PENDING_SUBAGENT_STATUSES` / `COMPLETED_SUBAGENT_STATUSES`, exhaustiveness-guarded by tests), and `TodoCompactionContextProvider` carries the agent's current todo list through summarization verbatim (see [Todo List](features/todo-list.md)). To add a contributor, implement the protocol and construct the system with `CompactionSystem(context_providers=[*DEFAULT_COMPACTION_CONTEXT_PROVIDERS, MyProvider()])`.
