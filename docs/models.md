...
- **Trim** permanently drops the oldest tool spans (atomic assistant-tool-call + results), then optionally strips replayed reasoning (`trim_reasoning`), until the estimate is under budget. It's cheap (no LLM) and rewrites history in place.
  - `protect_recent_turns: int` (default `100`) keeps the most recent N messages untouched (tool spans reaching into them are kept whole; their reasoning is not stripped), so short conversations are never trimmed and only long histories are. Set `0` to allow trimming up to the newest message.
